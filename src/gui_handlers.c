...
void
expose_event (GtkWidget *drawing_area, GdkEventExpose *event, gpointer data)
{
	GdkPixbuf *pixbuf;
	GtkStyle *style;
	GtkWidget *parent;
	...
	parent = (GtkWidget *)g_object_get_data (G_OBJECT (drawing_area), "parent");
	style = gtk_widget_get_style(parent);
	color = ((style->bg[0].red/256) << 16) + ((style->bg[0].green/256) << 8)\
		+ (style->bg[0].green/256 ) ;
...
//				(double) drawing_area->allocation.height / gdk_pixbuf_get_height (pixbuf),
				...
				event->area.x, event->area.y, 16, color, color);
...
void
config_event (GtkWidget *drawing_area, GdkEventConfigure *event, gpointer data)
{
	GdkPixbuf *pixbuf;
