...
{
	static void (*common_rtv_loader)(gconstpointer *,ConfigFile *,gchar * section, ComplexExprType);
	...
	for (i=0;i<total_symbols;i++)
	{
		...
		{
			case ECU_EMB_BIT:
				common_rtv_loader(object,cfgfile,section,ECU_EMB_BIT);
				break;
			case ECU_VAR:
				common_rtv_loader(object,cfgfile,section,ECU_VAR);
				break;
			case RAW_VAR:
				/* RAW variable */
				name=NULL;
			...
			default:
				dbg_func(RTMLOADER|COMPLEX_EXPR|CRITICAL,g_strdup_printf(__FILE__": load_complex_params(), expr_type %i is UNDEFINED, this will cause a crash!!\n",expr_types[i]));
		}
	}
}
