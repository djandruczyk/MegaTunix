...
//		fd = _open_osfhandle(serial_params->comm_handle, 0);
	...
	if (fd > 0)
	{
		...
		port_open = TRUE;
		serial_params->fd = fd;
		MTXDBG(SERIAL_RD|SERIAL_WR,g_strdup_printf(__FILE__" open_serial()\n\t%s Opened Successfully\n",port_name));
	...
	else
	{
		...
		MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL,g_strdup_printf(__FILE__": open_serial()\n\tError Opening \"%s\", Error Code: \"%s\"\n",port_name,err_text));
...
G_MODULE_EXPORT void setup_serial_params(void)
{
	...
	if (!parse_baud_str(baud_str,&baudrate,&bits,&parity,&stop))
		MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": setup_serial_params()\tERROR! couldn't parse ecu_baud string %s\n",baud_str));
	...
	*/
	...
	if (ioctl(serial_params->fd, TIOCGSERIAL, &serial_params->oldctl) != 0)
		MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": setup_serial_params()\tError getting serial line information error %s\n",strerror(errno)));
	else
	{
		MTXDBG(SERIAL_RD|SERIAL_WR, g_strdup_printf(__FILE__": setup_serial_params()\tget serial line info call OK\n"));
		...
		if (baudrate == 8192)
		{
			...
			if (ioctl(serial_params->fd, TIOCSSERIAL, &serial_params->newctl) != 0)
				MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": setup_serial_params()\tError setting ioctl\n"));
			else
				MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": setup_serial_params()\tset ioctl OK\n"));
		}
	}
	*/
...
G_MODULE_EXPORT void close_serial(void)
{
	...
	if (ioctl(serial_params->fd, TIOCSSERIAL, &serial_params->oldctl) != 0)
		MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": close_serial()\tError restoring serial line config\n"));
	else
		MTXDBG(SERIAL_RD|SERIAL_WR, g_strdup_printf(__FILE__": close_serial()\tserial line settings restored OK\n"));
	*/
	...
	MTXDBG(SERIAL_RD|SERIAL_WR,g_strdup(__FILE__": close_serial()\n\tSerial Port Closed\n"));
...
G_MODULE_EXPORT gboolean parse_baud_str(gchar *baud_str, gint *baud, gint *bits, Parity *parity, gint *stop)
{
	...
	{
		MTXDBG(SERIAL_RD|SERIAL_WR|CRITICAL, g_strdup_printf(__FILE__": pause_baud_str()\tbaud string is NOT in correct format 'baud,bits,parity,stop'\n"));
		g_strfreev(vector);
		return FALSE;
	}
	if (baud)
