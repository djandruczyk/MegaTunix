...
/*	printf("Number of chars received: %i\n",res); */
	if (res != serial_params.raw_bytes) /* the number of bytes expected for raw data read */
	{
	/*	printf("SERIAL I/O read error, flushing port\n"); */
		/* Serial I/O problem, resetting port 
		 * This problem can occur if the MS is resetting due to
		 * power spikes, or other problems with the serial
		 * interface, perhaps even a very slow machine
		 * The problem is part of the data is lost , and since we read
		 * in serial_params.raw_bytes blocks, the data is now 
		 * offset can hoses things all to hell.  The solution is 
		 * to close the port, re-open it and reinitialize it, 
		 * then continue.  We CAN do this here because the serial 
		 * I/O thread depends on this function and blocks until 
		 * we return.
		 */
	...
	if ((lastcount - raw->secl > 1) && (lastcount - raw->secl != 255))
	{	/* Counter jump, megasquirt reset occurred */
//		printf("Megasquirt reset occurred %i, %i\n",raw->secl,lastcount);
		reset_count++;
	}
	lastcount = raw->secl;
