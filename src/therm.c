...
#define Tu(f,celsius) (celsius ? ((f-32.0)*5.0/9.0) : f)
	...
	tim = g_malloc(sizeof(time_t));
	time(tim);

	FILE *f = fopen("/tmp/thermistor.log", "w");
	fprintf(f, "//------------------------------------------------------------------------------\n");
