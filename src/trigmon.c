...
gboolean trigmon_swap_labels(GtkWidget *widget, GdkEvent *event, gpointer data);


/*!
 \brief finish_trigmon() completes the trigmon view by populating the
 ...
 */
EXPORT void finish_trigmon(void)
{
	GtkWidget *sw = NULL;
	GtkWidget *hbox = NULL;
	GtkWidget *vbox = NULL;
	...
	top = g_hash_table_lookup(dynamic_widgets,"trigmon_top_vbox1");
	if (!GTK_IS_WIDGET(top))
		return;

	frame = gtk_frame_new("MSnS-E Trigger monitor");
	...
	lbl_table = gtk_table_new((firmware->page_params[firmware->trigmon_page]->length)/cols,1,TRUE);
	...
	g_signal_connect(G_OBJECT(ebox),"button_press_event",
			G_CALLBACK(trigmon_swap_labels),NULL);

	table = gtk_table_new(((firmware->page_params[firmware->trigmon_page]->length)/cols)+1,cols,TRUE);
	gtk_box_pack_start(GTK_BOX(hbox),table,TRUE,TRUE,5);
	col = 0;
	for (j=0;j<cols;j++)
	{
		label = gtk_label_new(g_strdup_printf("0x%.2X",j));
		gtk_table_attach(GTK_TABLE(table),label,j,j+1,0,1,
	...
	col = 0;
	row = 1;
	for (j=0;j<firmware->page_params[firmware->trigmon_page]->length;j++)
	{
		entry = gtk_entry_new();
		/*
		   ...
		   */
		   ...
		   ve_widgets[firmware->trigmon_page][j] = g_list_append(ve_widgets[firmware->trigmon_page][j],(gpointer)entry);
		   /*
...
/*!
 \brief trigmon_swap_labels() swaps the address labels between hex or decimal
...
void trigmon_update_view()
{
	printf("TRIGMON update view. not written yet\n");
}
