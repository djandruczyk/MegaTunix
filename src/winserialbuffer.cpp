...
void CSerialBuffer::LockBuffer()
{
	g_mutex_lock(lock);
}

void CSerialBuffer::UnLockBuffer()
{
	g_mutex_unlock(lock);
}
...
gboolean CSerialBuffer::Read_Available( std::string &Data,HANDLE & EventToReset)
{
...
gboolean CSerialBuffer::Read_Upto( std::string &Data,char chTerm,long  &bytes_read ,HANDLE & EventToReset)
{
	...
	{//if there are some bytes un-read...
			...
			{ 
				//if we are here it means that there is still some data in the loc buffer and
				//we have ready found what we want... maybe this is ok but we want to catch this
...
gboolean CSerialBuffer::Read_Upto_FIX( std::string &Data,char chTerm,long  &bytes_read ,HANDLE & EventToReset)
{
	MTXDBG (CRITICAL,_("CSerialBuffer : (tid:%d) Read_Upto cled "), GetCurrentThreadId ());
